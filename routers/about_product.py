...
from schemas.products.AboutProductRead import AboutProductRead
...
async def about_product(request: Request, product_slug: str, db: Session = Depends(get_db)):
    product = (db.query(Offer).options(selectinload(Offer.images)).filter(Offer.slug == product_slug).first())
    if product is None:
